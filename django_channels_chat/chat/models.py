...
class Group(models.Model):
    """модель, в которой пользователи будут обсуждать что-либо"""
    ...
    def add_user_to_group(self, user: User):
        """метод для добавления пользователя в группу и создания объекта"""
        self.members.add(user)
        self.event_set.create(type="Join", user=user)
        self.save()

    def remove_user_from_group(self, user: User):
        """метод для удаления членов группы, когда они выходят из группы чата"""
...
class Message(models.Model):
    """модель, отвечающая за сообщение"""
...
class Event(models.Model):
    """модуль хранящая информацию о конкретной группе"""

    CHOICES = [("Join", "join"), ("Left", "left")]
    type = models.CharField(choices=CHOICES, max_length=10)
    description = models.CharField(
        help_text="Дополнительное описание события", max_length=50, editable=False
    )
