...
def home_view(request):
    """представление главной страница, на которой перечислены все группы"""
...
def group_chat_view(request, uuid):
    """представление для группы, сообщения и события отправляются на интерфейс"""

    group = get_object_or_404(Group, uuid=uuid)
    if request.user not in group.members.all():
        return HttpResponseForbidden(
            "Вы не являетесь пользователем данной группы.\
                                       Пожалуйста, воспользуйтесь кнопкой 'Присоединиться'"
        )
